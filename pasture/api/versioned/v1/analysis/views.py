...
from .serializers import (
    CorrInputSerializer,
    DistanceListSerializer,
    PortfolioRowSerializer,
    PerformanceInputSerializer,
    ...
    def get_universe(self, **kwargs):
        ...
        if universe_count > 1:
            raise exceptions.ValidationError(
                {"detail": "matched universe must be unique"}
            )
        if universe_count == 0:
...
class CorrelationViewSet(
    ...
    serializer_class_map = {"create": CorrInputSerializer}
    queryset_map = {"get": AssetUniverse.objects.all()}
    ...
    def get(self, request, *args, **kwargs):
        ...
        _sorted_dist = self._cluster.get_sorted_corr(
            corr=dist.value, indices=diag_indices
        )
        w = self._cluster.get_recursive_bisect(corr=corr, sort_ix=diag_indices)
        w.index = corr.columns[w.index]
        asset_description = {
            i["symbol"]: i for i in Asset.objects.filter(symbol__in=w.index).values()
        }
        distance_serializer = DistanceListSerializer(data=list(_sorted_dist.iterrows()))
        asset_serializer = PortfolioRowSerializer(
            data=[{"symbol": i, "weight": round(w * 100, 3)} for i, w in w.iteritems()],
        ...
        return Response(
            {"distances": distance_serializer.data, "portfolio": asset_serializer.data}
        )
...
class PerformanceViewSet(
    SerializerMapMixin, QuerysetMapMixin, UniverseLookupMixin, viewsets.ModelViewSet
):
    ...
    def calc_metric(self, request, *args, **kwargs):
        ...
        port = Portfolio(
            weights={
                w["symbol"]: w["weight"] for w in serializer.validated_data["portfolio"]
            }
        )
        ...
        queryset = self.queryset.filter(
            symbol__in=port.symbols + serializer.validated_data["bench_marks"],
            **filter_kwargs
        )
        asset_profiler = AssetProfiler(
            bm_ticker=serializer.validated_data["bench_marks"][0]
        )
        ...
        output_serializer = MetricOutputSerializer(
            {"metrics": profiled.T.reset_index().to_dict(orient="records")}
        )
    ...
    def backtest_portfolio(self, request, *args, **kwargs):
        ...
        return Response(
            [{"x": i, "y": row} for i, row in portfolio_returns.iteritems()]
        )
